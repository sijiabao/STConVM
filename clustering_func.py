...
def mclust_R(adata, n_clusters, use_rep='SEDR', key_added='SEDR' ,random_seed = 2023,refine=False):
    """\
    Clustering using the mclust algorithm.
    The parameters are the same as those in the R package mclust.
    """
    import os
    os.environ['R_HOME'] = 'D:/R/R-4.3.0'
    modelNames = 'EEE'
    ...
    adata.obs[key_added] = adata.obs[key_added].astype('category')
    if refine:
        new_type = refine_label(adata, radius=90, key='SEDR')
        adata.obs['SEDR'] = new_type
